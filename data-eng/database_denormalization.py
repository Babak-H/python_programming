...
# this is the exact reverse of the normalization process, instead of making tables smaller, make them bigger
...
# select everything via join to see all the data
try:
...
# De-normalize the tables:

# here we added back name of the artist to album_library (therefore no need for artist_library table anymore)
try:
    cur.execute("CREATE TABLE IF NOT EXISTS album_library1 (album_id int, album_name varchar, \
                                                            artist_name varchar, year int);")
except psycopg2.Error as e:
    print("Error: Issue creating table")
    print(e)

# here we added back length of the song to songs_lib (therefore no need to song_length table anymore)
try:
...
# querying and joining new tables
try:
...
# this table contains id's from songs, names from album albums and length of each song
try:
...
# we can easily get length of each album via a group by
try:
    cur.execute(
        "SELECT album_name, SUM(song_length) FROM album_length GROUP BY album_name;")
except psycopg2.Error as e:
    print("Error: select *")
    print(e)
